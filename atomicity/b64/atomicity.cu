...
__global__ void hopper_v4_verify_kernel(uint32_t* global_out) {
    ...
    for (int i = tid; i < SMEM_UINT32_COUNT; i += block_size) {
    ...
    // --- Warp 1: 对 addr0 执行 st.shared.v4.u32 (128-bit) ---
        ...
        if (lane_id == 0) {
	    unsigned long long val_low  = 0x0000000100000002ULL;

            asm volatile (
                "st.shared.b64 [%0], %1;"
                : : "r"(smem_ptr_32), "l"(val_low) : "memory"
            );
        }
    }
    // --- Warp 2: 对 addr0 执行 ld.shared.v4.u32 (128-bit) ---
        ...
        if (lane_id == 0) {
            uint32_t r0=0, r1=0;
            unsigned long long r_low=0;

	    while(r0 != 0x00000001 || r1 != 0x00000002){
	    	asm volatile (
                	"ld.shared.b64 %0, [%1];"
                	: "=l"(r_low) : "r"(smem_ptr_32)
            	);
