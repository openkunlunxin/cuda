...
__global__ void hopper_v4_verify_kernel(uint32_t* global_out) {
    ...
    if (warp_id == 0) {
        ...
        for (int i = 0; i < 500000; ++i) {
            uint32_t idx = 4 + ((lane_id + i * 32) % (SMEM_UINT32_COUNT - 4)) / 4 * 4 ;
            //s_data[idx] = 0xDEADC0DE;
	    s_data[idx+1] = s_data[idx];
	    // s_data[idx+1] = 0xDEADC0DE;
        }
    }
   ...
   // --- Warp 1: 对 addr0 执行 st.shared.v4.u32 (128-bit) ---
	    ...
	    while(r0 != 0x00000011){
	    	asm volatile (
                	"ld.shared.v4.u32.volatile {%0, %1, %2, %3}, [%4];"
    ...
    // --- Warp 1: 对 addr0 执行 st.shared.v4.u32 (128-bit) ---
            ...
            uint32_t src0 = 0x00000011;
            uint32_t src1 = 0x00000022;
            uint32_t src2 = 0x00000033;
            uint32_t src3 = 0x00000044;

            asm volatile (
                "st.shared.v4.u32.volatile [%0], {%1, %2, %3, %4};"
...
int test() {
    ...
    // 4. 验证数据
    uint32_t expected[4] = {0x00000011, 0x00000022, 0x00000033, 0x00000044};
    bool success = true;
