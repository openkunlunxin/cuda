...
__global__ void device_memst_kernel(int* dst, int* src, size_t N) {
    ...
    if (idx == 0) {
		...
		*/
		cuda_scope::store_sys_relaxed(&dst[idx+0 ], tmp0);
		cuda_scope::store_sys_relaxed(&dst[idx+32], tmp0);
		cuda_scope::store_sys_relaxed(&dst[idx+2 ], tmp0);
		cuda_scope::store_sys_relaxed(&dst[idx+3 ], tmp0);
