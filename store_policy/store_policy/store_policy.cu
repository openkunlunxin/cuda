...
__global__ void device_memcpy_kernel(int* dst, int* src, size_t N) {
    ...
    if (idx < N) {
        ...
        __stcg(&src[idx+0], src0);
        __stcg(&src[idx+1], src1);
        __stcg(&src[idx+2], src2);
        __stcg(&src[idx+3], src3);
        // src[idx] = dst[idx] + 1;
	    // src[idx+1] = dst[idx+1] + 2;
        // src[idx+2] = dst[idx+2] + 3;
        // src[idx+3] = dst[idx+3] + 4;
    }
}


int main() {
    const size_t N = 16 * (1<<10); // 128KB
