...
__global__ void add_one_kernel(int* src, int* dst)
{
  ...
  if (idx < N) {
      ...
      // const int* addr = src+idx+N;
      // int value = __ldcv(addr);
      int value;
      asm volatile("ld.global.L2::64B.b32 %0, [%1];" 
                    : "=r"(value) 
                    : "l"(src+idx));
      dst[idx] = value+1;
  }
  ...
  if (threadIdx.x == 0) {
    ptx::cp_async_bulk(
        ptx::space_global,
        ptx::space_shared,
        src+N, smem_data, sizeof(smem_data));
...
int main() {
    ...
    // host memory alloc
    h_src = (int*)malloc(2*N * sizeof(int));
    h_dst = (int*)malloc(2*N * sizeof(int));

    for (size_t i = 0; i < 2*N; ++i)
        h_src[i] = static_cast<int>(i+10);

    for (size_t i = 0; i < 2*N; ++i)
	h_dst[i] = static_cast<int>(i+100);
    
    // device memory alloc
    CHECK_CUDA(cudaMalloc(&d_src, 2*N * sizeof(int)));
    CHECK_CUDA(cudaMalloc(&d_dst, 2*N * sizeof(int)));

    // host2device: init src
    CHECK_CUDA(cudaMemcpy(d_src, h_src, 2*N * sizeof(int), cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMemcpy(d_dst, h_dst, 2*N * sizeof(int), cudaMemcpyHostToDevice));
    ...
    CHECK_CUDA(cudaMemcpy(h_dst, d_dst, 2*N * sizeof(int), cudaMemcpyDeviceToHost));
    CHECK_CUDA(cudaMemcpy(h_src, d_src, 2*N * sizeof(int), cudaMemcpyDeviceToHost));
