...
#define CONST_DATA_MAX (64 * 1024 / sizeof(int))
...
void measureMemoryLatency() {
    ...
    // int test_sizes2[] = {3 * 1024};
    
    //for (int size_kb : test_sizes) {
    for (int size_kb=128; size_kb<=64*1024; size_kb+=128){
	...
	// copy const
	cudaMemcpyToSymbol(constData, h_constData, sizeof(float) * elements);
