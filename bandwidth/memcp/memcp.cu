...
__global__ void device_memcpy_kernel(int* dst, const int* src) {
    ...
    for (int i = 0; i < UNROLL_SIZE/VEC_SIZE; i ++) {
        y_vec[thrx + blockDim.x * gridDim.x * i] = x_vec[thrx + blockDim.x * gridDim.x * i];
    }
}
