...
__global__ void device_memcpy_kernel(int* dst, const int* src, const int n) {
    ...
    if(thrx/8 < threads_needed && thrx%8==0){
        #pragma unroll
        for (int i = 0; i < UNROLL_SIZE/VEC_SIZE; i ++) {
            y_vec[thrx/8 + blockDim.x * gridDim.x * i] = x_vec[thrx/8 + blockDim.x * gridDim.x * i];
        }
    }
}
