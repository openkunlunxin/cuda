...
__global__ void race_condition_kernel(int* global_addr, int* result_out) {
    ...
    *(volatile int*)global_addr = 1;

    // 3. 发射异步拷贝: Global -> Shared
    // 我们想看看这一步捕获的是 1 还是 2
    __pipeline_memcpy_async(&s_data, global_addr, sizeof(int));
    __pipeline_commit();
    
    // 4. 第二次 Store: 立即将 global_addr 修改为 2
    *(volatile int*)global_addr = 2;
    }
    // 5. 等待异步拷贝完成
...
int test() {
    ...
    // 分配显存
    cudaMalloc(&d_addr, sizeof(int));
    cudaMalloc(&d_result, sizeof(int));

    cudaMemcpy(d_addr, &h_addr, sizeof(int), cudaMemcpyHostToDevice);
