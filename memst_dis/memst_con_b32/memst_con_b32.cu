...
__global__ void device_memcpy_kernel(int* dst, const int n) {
    ...
    if( thrx < threads_needed){
        for (int i = 0; i < UNROLL_SIZE; i ++) {
            dst[blockDim.x * gridDim.x * i + thrx] = 1;
        }
    }
}
...
int main() {
    ...
    device_memcpy_kernel<<<blocks, threads>>>(d_dst, N);
